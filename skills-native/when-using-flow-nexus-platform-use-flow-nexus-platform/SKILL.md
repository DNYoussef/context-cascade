...
### 1. Authentication First, Operations Second
Every platform interaction requires valid authentication. Never attempt service operations before verifying session status.

**In practice:**
- Always check auth status before workflow execution
- Store user IDs in memory for session persistence
- Implement auto-refresh for token expiration
- Never commit credentials to version control

### 2. Incremental Service Configuration
Configure platform services progressively rather than all at once. Start with authentication, then sandboxes, then storage/databases, then deployment.

**In practice:**
- Complete Phase 1 authentication before Phase 2 services
- Validate each service before moving to next
- Store service IDs in memory for cross-phase coordination
- Use hooks to track phase completion

### 3. Cost-Aware Resource Management
Monitor credits, configure auto-refill, and optimize resource usage to prevent service interruption.

**In practice:**
- Check balance before expensive operations
- Enable auto-refill for production environments
- Monitor usage patterns in audit logs
- Clean up unused sandboxes and storage

## Anti-Patterns

| Anti-Pattern | Problem | Solution |
|--------------|---------|----------|
| **Skipping Authentication Verification** | Assumes credentials are valid without checking, leading to failed operations mid-workflow | Always run auth_status check before service operations |
| **Hardcoding Credentials** | Credentials committed to git expose security risks and violate best practices | Use environment variables and .gitignore for sensitive data |
| **No Resource Cleanup** | Abandoned sandboxes accumulate costs, wasting credits on unused services | Implement lifecycle management with sandbox_stop after tasks |
| **Missing Error Handling** | Network failures or API errors cause silent failures without retry logic | Add try-catch with exponential backoff for MCP operations |
| **Single-Phase Configuration** | Attempting all services simultaneously creates dependency conflicts | Follow 5-phase sequential workflow for proper setup |

## Conclusion

The Flow Nexus Platform skill provides comprehensive cloud infrastructure management through a systematic 5-phase workflow. By following the authentication-first principle, practitioners ensure stable service configuration before deploying applications. The phase structure (Authentication -> Services -> Deployment -> Operations -> Billing) creates natural dependency management and reduces configuration errors.
